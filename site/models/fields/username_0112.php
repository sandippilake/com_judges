...
class JFormFieldUsername extends JFormFieldList
{
	 ...
	 protected function getOptions()
	{
		...
		$query = "SELECT distinct c.user_id as value,CONCAT(c.firstname,' ',c.lastname) as text FROM `#__comprofiler` as c WHERE not exists (select j.* from #__jdg_judges as j where j.user_id = c.user_id)order by c.lastname ASC";
		$db->setQuery($query);
